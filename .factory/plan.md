...
sources (including one local Markdown-file import), select sources, preview
the exact Markdown before a release is saved, copy/download released packs,
and flag a release stale after a manual source-revision edit. The live
application has `/`, `/demo`, `/workspace`, `/privacy`, `/terms`, `/health`,
and a designed 404. Demo state is in the browser session and real-workspace
...
free-first-release behavior. The current repair provides a read-only exact
draft preview before any write for both demo and real workspace; the final
release action persists the exact previewed bytes only after explicit
confirmation. The real path compiles through one server-owned compiler; demo
uses a deterministic local equivalent, and tests compare preview,
released/downloaded bytes.
